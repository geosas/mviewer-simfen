mviewer.customControls.calcGhosh = (function() {
    /*
     * Private
     */
    var _idlayer = 'calcGhosh';
    ...
    var _showResult = function (data) {
        ...
        mviewer.customLayers.calcGhosh.layer.getSource().addFeature(feature);
    };
    ...
    return {
        ...
        calcul: function () {
            ...
            if (!_xy || (times.length === 0)) {
                mviewer.alert("calcGhosh : Il faut définir l'origine et au moins un temps de parcours", "alert-info")
                return;
            }
            mviewer.customLayers.calcGhosh.layer.getSource().clear();
            var mode = $(".selected.isochrone-mode").attr("data-mode");
            var url = "http://kartenn.region-bretagne.fr/isochrone?";

            $("#loading-calcGhosh").show();
